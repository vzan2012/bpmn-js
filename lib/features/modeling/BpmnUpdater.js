...
export default class BpmnUpdater extends CommandInterceptor {
  ...
  constructor(eventBus, bpmnFactory, connectionDocking, translate) {
  ...
  updateBounds(shape) {
    ...
    var bounds = (shape instanceof Label) ? this._getLabelBounds(di) : di.bounds;
  ...
  updateFlowNodeRefs(businessObject, newContainment, oldContainment) {
  ...
  updateDiConnection(di, newSource, newTarget) {
  ...
  updateSemanticParent(businessObject, newParent, visualParent) {
  ...
  _getLabelBounds(di) {
    ...
    if (!label) {
      di.label = label = this._bpmnFactory.createDiLabel();
    }
    ...
    if (!bounds) {
      label.bounds = bounds = this._bpmnFactory.createDiBounds();
    }
